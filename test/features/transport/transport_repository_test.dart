...
void main() {
  ...
  test(
    'persistChaine enregistre, calcule distance et journalise la sync',
    () async {
      final chaine = [
  ...
  test('persistChaine enregistre ET relit les photos', () async {
    await repo.persistChaine('MICA-2026-0001', [
      const Transbordement(
        ...
        photoDechargeHeadingDegrees: 45,
        photoDechargeHeadingAccuracy: 2,
        photoDechargeHeadingReference: 'magnetic',
        photoRechargePath: '/tmp/reload.jpg',
        photoRechargeHeadingDegrees: 225,
        photoRechargeHeadingAccuracy: 4,
        photoRechargeHeadingReference: 'magnetic',
      ),
    ]);
    final back = await repo.chaineFor('MICA-2026-0001');
    expect(back.single.photoDechargePath, '/tmp/unload.jpg');
    expect(back.single.photoDechargeHeadingDegrees, 45);
    expect(back.single.photoDechargeHeadingAccuracy, 2);
    expect(back.single.photoDechargeHeadingReference, 'magnetic');
